...
contract Library {
  ...
  event BookUploaded(uint256 book_Id,string book_Hash,uint256 book_Size,string book_Name,string book_Description,string author,uint256 uploadTime,address uploader);
  ...
  function deletebook(uint index) public onlylibrarian(){
      ...
      if(index != BookCount){
        books[index]= Book(index,books[BookCount].book_Hash,books[BookCount].book_Size,books[BookCount].book_Name,books[BookCount].book_Description,books[BookCount].author,books[BookCount].uploadTime,books[BookCount].uploader);
        books[BookCount] = Book(0, "", 0, "", "", "", 0, address(0));
        BookCount --;}
      else
      {
        books[index] = Book(0, "", 0, "", "", "", 0, address(0));
        BookCount--;
      }
    }
    
  function uploadBook(string memory _book_Hash,uint256 _book_Size,string memory _book_Name,string memory _book_Description,string memory _author) public onlylibrarian() {
    ...
    books[BookCount] = Book(BookCount,_book_Hash,_book_Size,_book_Name,_book_Description,_author,now,msg.sender);

    emit BookUploaded(BookCount,_book_Hash,_book_Size,_book_Name,_book_Description,_author,now,msg.sender);
  }
